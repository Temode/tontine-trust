-- =====================================================================
-- P0.2 — Pénalités de retard
--   1. Colonne contributions.penalty_amount
--   2. record_mock_payment : calcule la pénalité et l'écrit au ledger
--   3. Vue my_contributions_due : expose expected_penalty
-- À exécuter dans le SQL Editor APRÈS db/15_fix_start_cycle_enum_cast.sql.
-- Idempotent.
-- =====================================================================

-- 1. Colonne pénalité figée au moment du paiement
alter table public.contributions
  add column if not exists penalty_amount bigint not null default 0
    check (penalty_amount >= 0);

-- 2. RPC record_mock_payment enrichi
...
begin
  ...
  -- Pénalité de retard : appliquée au-delà du délai de grâce.
  ...
  -- 1) tentative de paiement (succès simulé), montant TTC pénalité incluse
  ...
  -- 2) contribution confirmée + pénalité figée
  ...
  -- 4) ledger : pénalité éventuelle (crédit groupe, comptabilisé séparément)
  if v_penalty > 0 then
    perform public.append_ledger(
      v_contrib.group_id, v_turn.cycle_id, v_turn.id, v_contrib.id, v_payment_id,
  ...
  -- 5) bascule du tour si toutes les cotisations sont reçues
...
-- 3. Vue : expose la pénalité projetée
