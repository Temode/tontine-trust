...
begin
  if tg_op = 'UPDATE' and OLD.status is distinct from NEW.status and not public.is_rpc_context() then
...
create trigger groups_block_direct_status before update on public.groups
...
declare v_uid uuid := auth.uid(); v_g public.groups%rowtype;
begin
  if v_uid is null then raise exception 'AUTH_REQUIRED'; end if;
  select * into v_g from public.groups where id = _group_id;
  if not found then raise exception 'GROUP_NOT_FOUND'; end if;
  if not public.has_admin_permission(_group_id, v_uid, 'can_pause_cycle') then raise exception 'FORBIDDEN'; end if;
  if v_g.status not in ('active', 'open') then raise exception 'NOT_PAUSABLE'; end if;
  perform set_config('app.via_rpc', '1', true);
  update public.groups set status='paused', paused_at=now(),
    paused_reason=_reason, paused_by=v_uid where id = _group_id;
  insert into public.notifications (user_id, kind, title, body, group_id, data)
  select gm.user_id, 'cycle_paused', 'Cycle mis en pause',
...
declare v_uid uuid := auth.uid(); v_g public.groups%rowtype; v_days int;
begin
  if v_uid is null then raise exception 'AUTH_REQUIRED'; end if;
  select * into v_g from public.groups where id = _group_id;
  if not found then raise exception 'GROUP_NOT_FOUND'; end if;
  ...
  perform set_config('app.via_rpc', '1', true);
  update public.groups set status='active', paused_at=null, paused_reason=null,
    paused_by=null, total_paused_days=coalesce(total_paused_days,0) + v_days where id = _group_id;
  if v_days > 0 then
    update public.turns set due_date = due_date + v_days
      where group_id = _group_id and status in ('upcoming', 'collecting');
  end if;
  insert into public.notifications (user_id, kind, title, body, group_id, data)
  select gm.user_id, 'cycle_resumed', 'Cycle relancé',
...
returns void language plpgsql security definer set search_path = public as $$
declare v_uid uuid := auth.uid(); v_turn public.turns%rowtype; v_old date;
begin
  if v_uid is null then raise exception 'AUTH_REQUIRED'; end if;
  ...
  insert into public.notifications (user_id, kind, title, body, group_id, data)
  select c.payer_user_id, 'due_date_shifted', 'Échéance reportée',
    format('L''échéance du tour #%s a été reportée au %s.', v_turn.turn_number, _new_date),
    v_turn.group_id, jsonb_build_object('turn_id', _turn_id, 'from', v_old, 'to', _new_date)
  from public.contributions c where c.turn_id = _turn_id and c.status <> 'confirmed';
