...
begin
  ...
  select gm.user_id,
         ...
         case when _approve
              then 'Tontine Digital a approuvé la suppression de "' || v_g.name || '". L''historique reste archivé pour 6 ans.'
              else 'Tontine Digital a refusé la suppression de "' || v_g.name || '"' || coalesce(' : ' || _reason, '.') end,
         v_req.group_id,
