alter table public.receipts
  add column if not exists fee_amount bigint not null default 0 check (fee_amount >= 0),
...
create or replace function public.release_payout(
  _turn_id uuid, _provider public.payment_provider default 'simulation'
) returns uuid
language plpgsql security definer set search_path = public as $$
declare
  v_user uuid := auth.uid(); v_turn public.turns%rowtype; v_remaining int;
...
create view public.my_receipts with (security_invoker = true) as
select r.id, r.receipt_number, r.turn_id, r.group_id, g.name as group_name,
  r.amount, r.fee_amount, r.net_amount, r.provider, r.hash, r.issued_at,
  t.turn_number, r.beneficiary_user_id, pb.full_name as beneficiary_name, pi.full_name as issued_by_name
